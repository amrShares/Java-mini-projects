...
public class InMemoryWorldDao implements WorldDao {
    ...
    private InMemoryWorldDao() {
		...
		/*
		• Create the above two classes and then create a java application to find the 
		following:
	...
	/*
	Collection<City> cities = allCities.values();
	Map<String, City> countryMax = cities.stream()
	.collect(Collectors.groupingBy(City::getCountryCode,
					Collectors.collectingAndThen(Collectors.maxBy(Comparator.comparing(City::getPopulation)),
									Optional::get)));
	...
	*/
	...
	.collect(Collectors.groupingBy(City::getCountryCode,
					Collectors.collectingAndThen(Collectors.maxBy(Comparator.comparing(City::getPopulation)),
									Optional::get)));
	Collection<City> maxCities = 	countryMax.values();
	
	Map<String, City> continentMax = maxCities.stream().collect(Collectors.groupingBy(s -> worldDao.findCountryByCode(s.getCountryCode()).getContinent(),
					Collectors.collectingAndThen(Collectors.maxBy(Comparator.comparing(City::getPopulation)),
									Optional::get)));
	System.out.println(continentMax);
	*/			
	

	/*
	Collection<Country> countries = allCountries.values();
	Map<String, Country> continentMax = countries.stream()
	.collect(Collectors.groupingBy(Country::getContinent,
					Collectors.collectingAndThen(Collectors.maxBy(Comparator.comparing(Country::getPopulation)),
									Optional::get)));
	System.out.println(continentMax);
	*/
	...
	City highestCapital = countries.stream()
				.map(Country::getCapitalCity).filter( (s) -> s != null)
				.max(Comparator.comparing((s) -> s.getPopulation())).orElse(null);
	System.out.println(highestCapital);
	
        //worldDao.writeCountriesToFile (countryMax, "Countries.txt");
        //worldDao.writeCitiesToFile (countryMax, "Cities.txt");
    }
    ///////////////////////////////////////////////////////////////////////////////
